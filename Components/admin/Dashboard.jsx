      ...
      <div className="max-w-7xl mx-auto space-y-6 md:space-y-8">
        ...
        <div className="grid grid-cols-1 gap-6 md:gap-8">
          ...
          <div className="bg-white rounded-[24px] md:rounded-[32px] p-4 sm:p-6 md:p-8 border border-pink-100/70 shadow-sm flex flex-col">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-5 pb-4 border-b border-pink-50">
              ...
              <button className="text-pink-600 hover:text-[#5D1232] font-black text-[10px] sm:text-xs uppercase tracking-wider flex items-center gap-1 transition-colors group self-end sm:self-auto">
                View All Orders
            ...
            {orders.length === 0 ? (
              <div className="py-12 text-center text-pink-400 font-medium text-xs sm:text-sm flex flex-col items-center justify-center gap-2">
                <Layers className="text-pink-200" size={28} />
                No active orders found in cache.
              </div>
            ) : (
              <div className="space-y-3 max-h-[480px] overflow-y-auto pr-0.5 custom-scrollbar">
                {orders.map((order, index) => (
                  ...
                  >
                    ...
                    <div className="flex sm:flex-col justify-between sm:justify-center items-center sm:items-end border-t sm:border-t-0 pt-2 sm:pt-0 border-pink-50/60 native-row-reverse-mobile">
                      <p className="font-black text-[#5D1232] text-sm sm:text-base tracking-tight">
                        ₹{order.total_price}
                      </p>

                      <span
                        className={`text-[8px] sm:text-[9px] flex items-center gap-1 font-black uppercase tracking-wider px-2 py-0.5 sm:py-1 rounded-md sm:rounded-lg mt-0.5 border ${
